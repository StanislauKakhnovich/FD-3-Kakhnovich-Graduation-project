...
class int_LaptopProductsInterface extends React.PureComponent {
  ...
  filterClick =() =>{
      ...
      if (arr1.length==0) this.setState( {iskl: true});
    
  }

  filterReset =() =>{
    let arr = [];
    this.setState( {controls:  arr, controlSort: 1, products: this.props.products, iskl: false});
  ...
  render() {
    ...
    return (
        <div className='ContainerProducts'>
          ...
          {
            this.state.iskl&&
            <h2 className='Registr'>Ничего не найдено.</h2>
          }
